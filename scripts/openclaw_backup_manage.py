...
def build_setup_plan_payload(settings: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "recommended_values": {
            ...
            "local_enabled": {"value": "on", "label": "开启"},
        ...
        "confirmation_rules": {
            "style": "batch-summary",
            "allow_accept_recommended": True,
            "core_confirm_together_ids": [
                "auto_enabled",
                "cadence",
                "check_weekday",
                "check_time",
            ...
            "follow_up_only_when_changed_or_missing": True,
            "conditional_groups": [
                {
                    "ids": [
                        "nas_root",
                        "warn_backup_root_gib",
                        "critical_backup_root_gib",
                    ],
                    "when": {"nas_enabled": "on"},
                    "reason": "开启离机归档时，必须确认稳定可写的 SMB 共享路径，以及对应的提醒阈值。",
                }
            ],
        },
        "product_logic": {
        ...
        "recommended_questions": [
            ...
            },
            {
                "id": "check_weekday",
                "question": "每周哪一天执行检查？",
                "recommended": weekday_label(int(auto["check"]["weekday"])),
                "recommended_value": str(int(auto["check"]["weekday"])),
                "required": True,
                "why": "检查日固定后，后续自动节奏更稳定、更容易记。",
            },
            {
                "id": "check_time",
                "question": "每周几点执行检查？",
                "recommended": str(auto["check"]["time"]),
                "recommended_value": str(auto["check"]["time"]),
                "required": True,
                "why": "固定检查时间后，更容易理解和核对自动执行窗口。",
            },
            {
                "id": "timezone",
                "question": "按哪个时区理解检查时间？",
                "recommended": str(auto["check"]["timezone"]),
                "recommended_value": str(auto["check"]["timezone"]),
                "required": True,
                "why": "明确时区后，定时任务不会因为时区误解而跑偏。",
            },
            {
                "id": "local_enabled",
                "question": "要不要开启本机快照？",
            ...
            },
            {
                ...
                "recommended_value": str(local["root"]),
                "required": True,
                "conditional_on": {"local_enabled": "on"},
            ...
            },
            {
                ...
                "recommended_value": str(local["keep"]),
                "required": True,
                "conditional_on": {"local_enabled": "on"},
            ...
            {
                "id": "nas_root",
                "question": "如果开启离机归档，归档目录是什么？",
                "recommended": "留空；只有提供稳定可写的 SMB 共享路径后再设置",
                "recommended_value": "",
                "required": False,
                "conditional_on": {"nas_enabled": "on"},
                "why": "开启离机归档时必须明确归档目录，否则不能安全启用。",
            },
            {
                "id": "warn_backup_root_gib",
                "question": "归档仓库多大时开始 warning 提醒？",
                "recommended": str(alerts["warn_backup_root_gib"]),
                "recommended_value": str(alerts["warn_backup_root_gib"]),
                "required": False,
                "conditional_on": {"nas_enabled": "on"},
            ...
            },
            {
                ...
                "recommended_value": str(alerts["critical_backup_root_gib"]),
                "required": False,
                "conditional_on": {"nas_enabled": "on"},
...
def human_setup_plan(payload: dict[str, Any]) -> str:
    ...
    lines = [
        "OpenClaw 备份初始化建议",
        "先把下面这份推荐配置一次性给用户确认；如果用户没意见，直接按这份落盘；只有用户要改，或开启离机归档但没给足信息时，再补问缺失项。",
        "",
        "推荐配置：",
        f"- 推荐自动备份：{recommended['auto_enabled']['label']}",
        f"- 推荐节奏：{recommended['cadence']['label']}",
        f"- 推荐检查星期：{weekday_label(recommended['check_weekday'])}",
        f"- 推荐检查时间：{recommended['check_time']}",
        f"- 推荐时区：{recommended['timezone']}",
        f"- 推荐本机快照：{recommended['local_enabled']['label']}",
        ...
        "- 若保持离机归档关闭：不要继续追问归档目录和提醒阈值",
        f"- 若开启离机归档：再确认 SMB 归档目录，以及 warning {recommended['warn_backup_root_gib']} GiB / critical {recommended['critical_backup_root_gib']} GiB",
        "",
        "解释：",
